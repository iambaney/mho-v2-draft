...
| Path | What it is |
|---|---|
| `content/home.yaml` | Every word on the page. Text changes happen here and nowhere else. |
...
| `src/components/` | One template per section: `Nav`, `Hero`, `Proof` (the stat cards), `Testimonials`, `Meet`, `QuoteBand`, `Footer`. Each is the HTML for that section with the content filled in. |
| `src/pages/index.astro` | The page. It loads the content file and stacks the sections in order. |
...
| `.pages.yml` | Configuration for Pages CMS, the form editor Anya uses. It writes to the same content file on `draft`. |
| `.github/workflows/` | Build, deploy and publish automation. |

## The flow for every change

1. Work on `draft`: `git switch draft && git pull`. Pull first, because Pages CMS commits to this branch too (its commits say "via Pages CMS").
2. Make the change.
...
- Change words in `content/home.yaml` only. Do not hard-code text into a template.
- Keep the file valid YAML. Values that start with a quote mark or contain `: ` are wrapped in double quotes, as the existing entries show.
...
- A new section is three things: a component in `src/components/`, a line in `index.astro` that renders it, and its text in `content/home.yaml`. Its styles go in `global.css` next to the others.
...
- No JavaScript on the page. No `<script>` tags, no `client:` directives, no Astro integrations or UI frameworks that ship code to the browser.
- No new npm dependencies without asking Anthony. The site has exactly two.
- Do not edit `.github/workflows/`, `.pages.yml` or `package.json` unless the task is about them.
